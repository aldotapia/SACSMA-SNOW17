...
def load_all_sacsma_parameters(forcing_type: str = "daymet"):

    # Construct file name from pieces
    filenames = glob.glob(
        f"{DATA_DIR}/model_output_{forcing_type}/model_output/flow_timeseries/{forcing_type}/**/*_model_parameters.txt"
    )
...
def load_sacsma_parameters(gauge_id: str, forcing_type: str = "daymet"):
    # Construct file name from pieces
    filenames = glob.glob(
        f"/home/aldotapia/data/{forcing_type}/model_output_{forcing_type}/model_output/flow_timeseries/{forcing_type}/**/{gauge_id}_*_model_parameters.txt"
    )
...
def load_basin_attributes(gauge_id: str):
    ...
    attributes_dir = Path(DATA_DIR) / "camels_attributes"
...
def load_forcings(gauge_id: str):
    ...
    forcing_files = glob.glob(
        f"/data/shared/CAMELS_US/basin_mean_forcing/daymet/**/{gauge_id}_*_forcing_leap.txt"
    )
    ...
    forcings["Date"] = pd.to_datetime(
        forcings["Year"] * 10000 + forcings["Mnth"] * 100 + forcings["Day"],
        format="%Y%m%d",
    )
    forcings = forcings.set_index("Date")

    return forcings, area, latitude, elevation
...
def load_discharge(gauge_id: str):
    ...
    # Grab the correct forcing file
    filenames = glob.glob(
        f"/home/aldotapia/data/daymet/model_output_daymet/model_output/flow_timeseries/daymet/**/{gauge_id}_*_model_output.txt"
    )
    ...
    output["Date"] = pd.to_datetime(
        output["YR"] * 10000 + output["MNTH"] * 100 + output["DY"], format="%Y%m%d"
    )
...
def load_usgs(gauge_id: str, area: int):
    ...
    # Grab the correct forcing file
    filename = glob.glob(
        f"/data/shared/CAMELS_US/usgs_streamflow/**/{gauge_id}_streamflow_qc.txt"
    )[0]
    ...
    obs["Date"] = pd.to_datetime(
        obs.Year.map(str) + "/" + obs.Mnth.map(str) + "/" + obs.Day.map(str)
    )
